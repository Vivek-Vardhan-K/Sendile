...
# Clean Objects in memory after session closes

@cross_origin()
@app.route('/removeToken/<token>', methods=['DELETE'])
def cleanObjectsInMemory(token):
    global userObjectStorage
    userObjectStorage.pop(token)
    return "token number: " + str(token) + " deleted"


def portBookToKindle(book_name, email):
